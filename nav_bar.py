...
const I18N_DICT = {
  zh: {
    ...
    // Factor Components
    factor_ZN_title: "10年期公債期貨(ZN)動能",
    factor_ZN_desc: "10年期公債期貨（ZN）近125個交易日（約半年）的累積報酬。上漲代表公債反彈（貪婪），急跌代表公債拋售（恐懼）。",
    factor_ZN_source: "資料來源：Yahoo Finance（ZN=F 每日收盤）",
    factor_ZN_leg1: "40日累積報酬 (%)",
    factor_ZN_leg2: "125日移動平均 (%)",
    factor_CG_title: "銅金比",
    factor_CG_desc: "銅期貨（HG=F）與金期貨（GC=F）收盤價的比值。數值高代表工業需求旺盛與景氣樂觀（貪婪）；數值低反映黃金避險需求強勁（恐懼）。",
    factor_CG_source: "資料來源：Yahoo Finance（HG=F 銅期貨、GC=F 金期貨每日收盤）",
    factor_CG_leg1: "銅金比原始數值",
    factor_CG_leg2: "5年滾動百分位分數",
  ...
  en: {
    ...
    // Factor Components
    factor_ZN_title: "10Y Treasury Futures (ZN) Momentum",
    factor_ZN_desc: "125-day cumulative return of 10-Year Treasury Futures (ZN). High returns indicate a bond rally (Greed); sharp drops reflect bond sell-offs (Fear).",
    factor_ZN_source: "Data Source: Yahoo Finance (ZN=F daily close)",
    factor_ZN_leg1: "40D Cumulative Return (%)",
    factor_ZN_leg2: "125D Moving Average (%)",
    factor_CG_title: "Copper / Gold Ratio",
    factor_CG_desc: "Ratio of Copper futures (HG=F) to Gold futures (GC=F). High ratio indicates economic expansion (Greed); low ratio reflects safe-haven demand (Fear).",
    factor_CG_source: "Data Source: Yahoo Finance (HG=F Copper, GC=F Gold daily closes)",
    factor_CG_leg1: "Copper/Gold Raw Ratio",
    factor_CG_leg2: "5-Year Rolling Percentile Score",
