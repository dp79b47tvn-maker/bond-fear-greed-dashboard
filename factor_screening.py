...
def render_registry_index(registry):
    ...
    for e in reversed(registry):
        ...
        rows += f"""<tr>
          <td>{e["test_date"]}</td>
          <td><a href="screening_{e['key']}.html">{e['label']}</a></td>
...
<div class="wrap">
  <header>
    <nav style="display:flex;gap:16px;flex-wrap:wrap;font-size:13px;margin-bottom:18px;">
      <a href="index.html" style="color:#1e3a5f;text-decoration:none;font-weight:600;padding:10px 6px;margin:-10px -6px;border-radius:6px;display:inline-block;">← 專案首頁</a>
      <a href="screening_form.html" style="color:#1e3a5f;text-decoration:none;font-weight:600;padding:10px 6px;margin:-10px -6px;border-radius:6px;display:inline-block;">➕ 線上驗證新因子</a>
    </nav>
    <div class="kicker">FACTOR SCREENING REGISTRY</div>
    <h1>因子篩選登記簿</h1>
    <p class="meta">累計測試 {n_total} 個因子，通過全部五關 {n_passed} 個。</p>
    <p class="disclaimer">
      這份登記簿記錄「每一次」測試過的因子，不只是成功的——刻意這樣做是為了防止多重比較的自欺：
      如果測了20個因子只有1個通過，這裡看得到實際命中率是1/20，不會只記得那1個成功案例、
      忘記自己已經試過19次。判定邏輯與門檻見<a href="AGENTS.md">AGENTS.md</a>。
    </p>
  </header>
  <section class="card">
